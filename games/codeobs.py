...
class CodeObservatory(GameBase):
    game_id = "codes"
    title = "Code Observatory"

    SNIPS = {
        "A": "print('Snippet A placeholder')",
        "B": "print('Snippet B placeholder')",
        "C": "print('Snippet C placeholder')",
        "D": "print('Snippet D placeholder')",
    }
    ...
    def mount(self, parent):
        ...
        self.text.insert("end", "Use: showcode A  (or B/C/D)\n")
        self.text.config(state="disabled")

    def show(self, key: str):
        key = (key or "").strip().upper()
        if key not in self.SNIPS:
            self.safe_print("[ERR] showcode expects A/B/C/D.")
            return
        self.current = key
        if not self.text:
            return
